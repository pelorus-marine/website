...
- **Rust** — CI pins **`1.90.0`** via **`dtolnay/rust-toolchain@master`** (same line as **`platform`** / **`specifications`**) and runs **`cargo fmt --all -- --check`**, **`cargo clippy … --all-targets -- -D warnings`**, **`cargo test`** / coverage with **`--all-features`** / **`--locked`** where configured.
