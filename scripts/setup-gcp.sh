...
echo ""
echo "This script does NOT create a Cloud Run service. After GitHub Actions secrets/variables match"
echo "the values above, push a version tag (v*) to run the release workflow, which deploys and creates:"
echo "  service name: ${CLOUD_RUN_HINT}   region: ${REGION}"
echo ""
echo "Or deploy manually once you have an image tag in Artifact Registry, e.g.:"
echo "  gcloud run deploy ${CLOUD_RUN_HINT} --project=${PROJECT_ID} --region=${REGION} \\"
echo "    --image=${AR_HOST}/${PROJECT_ID}/${ARTIFACT_REPO}/website:YOUR_TAG \\"
echo "    --allow-unauthenticated --port=8080"
echo ""
echo "Cloud Run custom domain mapping in ${REGION} is preview; global HTTPS LB works everywhere."
echo ""
